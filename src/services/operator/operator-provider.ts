...
export class OperatorProvider extends BaseOperatorProvider {
  ...
  async getTransactionProof(encoded: string): Promise<Proof> {
    ...
    const deposits: Deposit[] = rawProof.deposits.map(
      (deposit: OperatorTransaction) => {
    ...
    const nonEmptyBlocks = Object.keys(rawProof.transactionHistory)
      .map((i) => {
        return Number(i)
      })
      .sort((a, b) => {
    ...
    const nonEmptyProofs: ProofElement[] = nonEmptyBlocks.reduce(
    ...
    const proof: ProofElement[] = nonEmptyProofs
      .concat(emptyProofs)
      .sort((a, b) => {
